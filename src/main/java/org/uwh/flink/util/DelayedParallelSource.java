 ...
 */
public class DelayedParallelSource<T> extends RichParallelSourceFunction<T> implements ResultTypeQueryable<T> {
    private SourceFunction<T> delegate;
    private TypeInformation<T> tinfo;
    private long millis;
    private static Logger logger  = LoggerFactory.getLogger(DelayedParallelSource.class);

    public DelayedParallelSource(SourceFunction<T> delegate, TypeInformation<T> tinfo, long millis) {
